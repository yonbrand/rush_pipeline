"""
Stage 4: Aggregation — compute subject-level summary statistics.

Reads bout-level and daily PA CSVs, produces one row per subject with:
  - Descriptive statistics for each gait measure (median, mean, std, percentiles, etc.)
  - Histogram features with global bin ranges
  - Variance-of-variance measures (within-bout variability of gait measures)
  - Daily walking amounts and step counts
  - Daily physical activity summaries

This is idempotent: re-run without re-running GPU inference.
"""
...
# Global ranges for histogram binning (ensures comparability across subjects)
GLOBAL_RANGES = {
...
# Columns to compute var-of-var on (require window-level data within bouts)
VAR_OF_VAR_WINDOW_METRICS = [
...
def aggregate_subject(
    ...
    Args:
        ...
        daily_pa: dict from preprocessing.compute_daily_pa().
    ...
    # --- Daily walking amount ---
    if not bout_df.empty and 'start_time' in bout_df.columns:
        try:
            bout_df_ts = bout_df.copy()
            bout_df_ts['start_time'] = pd.to_datetime(bout_df_ts['start_time'])
            bout_df_ts['day'] = bout_df_ts['start_time'].dt.date
            daily_walking_min = bout_df_ts.groupby('day')['duration_sec'].sum() / 60
            row.update(_calc_stats(daily_walking_min.values, 'daily_walking_'))
        except Exception:
            row.update(_calc_stats(np.array([]), 'daily_walking_'))
    else:
        row.update(_calc_stats(np.array([]), 'daily_walking_'))
    ...
    if not bout_df.empty and 'start_time' in bout_df.columns:
        try:
            ...
            bout_df_ts['start_time'] = pd.to_datetime(bout_df_ts['start_time'])
            bout_df_ts['day'] = bout_df_ts['start_time'].dt.date
            daily_steps = bout_df_ts.groupby('day')['total_steps'].sum()
            row.update(_calc_stats(daily_steps.values, 'daily_step_count_'))
        except Exception:
            row.update(_calc_stats(np.array([]), 'daily_step_count_'))
    else:
        row.update(_calc_stats(np.array([]), 'daily_step_count_'))
    ...
    # --- Variance-of-variance (within-bout variability from window data) ---
    row.update(_var_of_var(window_df, bout_df))
...
def _var_of_var(window_df: pd.DataFrame, bout_df: pd.DataFrame) -> dict:
    """
    Compute variance-of-variance: the spread of within-bout variability.

    For each gait metric, compute the std within each bout (from window data),
    then report the median and std of those per-bout stds across all bouts.
    """
    result = {}

    if window_df.empty or bout_df.empty:
        for metric in VAR_OF_VAR_WINDOW_METRICS:
...
# ============================================================================
# Batch aggregation (process pre-saved bout/window CSVs)
# ============================================================================

def aggregate_from_csvs(output_dir: str) -> pd.DataFrame:
    """
    Re-run aggregation from previously saved bout/window/daily_pa CSVs.

    This allows modifying statistics without re-running GPU inference.
